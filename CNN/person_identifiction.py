    ...
    for dir in directories:
        for folder in os_sorted(os.listdir(path+dir)):
            if folder.startswith("GEI"):
                ...
                label+=1
    shuffle(Data)
    # np.save('personal_identification_data.npy', Data)
    return Data, label-1

data,label = create_train_data(Data_DIR)
...
#Here we use softmax activation function.
model.add(keras.layers.Dense(label+1,activation='softmax'))
...
check = ModelCheckpoint('person_best_model.h5', monitor='val_acc', mode='max', verbose=1, save_best_only=True)


Model = model.fit_generator(dataAugmentaion.flow(X, Y,batch_size=20), validation_data = (test_x, test_y),
steps_per_epoch = len(X) //5, epochs = 500, callbacks=[stop,check], verbose= 1)

#Model valdation and testing:

Load_model = load_model('person_best_model.h5')
...
score_personal= Load_model.evaluate(X_val, Y_val, verbose=0)
